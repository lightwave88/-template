...
// 把文本解析成藥的樣板
import { AnalyzeEngine } from './template/analyzeContent_1.js';
...
// out 模組
const $out_addModules = OutputModuleClass.addModules;
...
const $template = (function () {
    ...
    const $template = function (content, options) {
        ...
        let al = new AnalyzeEngine(content, options);
    ...
    (function () {
        ...
        $template.addOutput = function (key, fn) {
            ...
            $out_addModules[key] = fn;
        };

        // 擴充一般功能
        $template.addFun = function (key, fn) {
            FuntionModels[key] = fn;
        };
