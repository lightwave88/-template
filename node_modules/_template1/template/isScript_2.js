...
const NodeClass = require('./node_1d.js');

const G = require('./globalVariable.js');
...
////////////////////////////////////////////////////////////////////////////////
// 要匯出的 funList
const CheckTag_methodList = {};

module.exports = CheckTag_methodList;
GModules["checkTag_methodList"] = CheckTag_methodList;
//------------------------------------------------
// 抽象類
class IsTag {
    constructor(root, head, body) {
        this.root = root;
...
const IsScript = (function () {
    ...
    class IsScript extends IsTag {
        ...
        _getNode() {
            ...
            let isCommand = Reg_4.test(s_head);

            if (isCommand) {
                node = new CommandNode(this.root, 'script', s_context);
                nodeList.push(node);
            ...
            if (!G.analyzeScript) {
                ...
                node = new ScriptPartNode(this.root, content, false);
                nodeList.push(node);
            } else {
        ...
        _findCommad(nodeList, context, isTag) {
            ...
            while ((res = reg_1.exec(context)) != null) {
                ...
                let methodClass = IsScript[tagName];
                if (methodClass == null) {
                ...
                let method = new methodClass(this.root, match, remain);
                let r = method.check();
                //-----------------------
                // debugger;
                // 是否有找到尾是正確的 tag
                ...
                if (r.find) {
                    ...
                    if (checked) {
                        node = new ScriptPartNode(this.root, checked, isTag);
            ...
            if (final) {
                node = new ScriptPartNode(this.root, final, isTag);
                // debugger;
        ...
        _getReg_1() {
            ...
            if (Reg_5.test(G.commandTagName)) {
                reg = Reg_6;
            } else {
                reg = Reg_7;
            }
...
CheckTag_methodList['script'] = IsScript;

//==============================================================================
// <% %>
const IsCommand_1 = (function () {

    let CheckReg_1; // 包括 include
    let CheckReg_2; // 不包括 include
    ...
    class IsCommand_1 extends IsTag {
        ...
        //--------------------------------------
        _init() {
            if (Reg_3.test(this.head)) {

                if (CheckReg_2 == null) {
                    CheckReg_2 = this._getCheckReg(Reg_2, false);
                }
                this.checkReg_1 = CheckReg_2;
            } else {
                if (CheckReg_1 == null) {
                    CheckReg_1 = this._getCheckReg(Reg_2, true);
                }
                this.checkReg_1 = CheckReg_1;
            }
        }
        //--------------------------------------
        _getCheckReg(endTag, checkInclude) {
            // debugger;
        ...
        _findEnd(content) {
            ...
            let originalContent = content.slice(0);
            ...
            while ((rgRes = this.checkReg_1.exec(content)) != null) {
                ...
                if (rgRes[1] != null) {
                    ...
                    //-------------
                    this.foot = rgRes[1];
                    this.textContent = originalContent.substr(0, hasChecked);

                    rValue.nodes = this._getNode(includes);

                    return rValue;
                } else if (rgRes[2] != null) {
                    // 找到 include() 命令
                    debugger;

                    let path = rgRes[2];

                    let j = match.search(Reg_4);
        ...
        //--------------------------------------
        _getNode(includes) {
            let node = new CommandNode(this.root, this.head, this.textContent, includes);
            return [node];
        }
        //--------------------------------------
    }
...
CheckTag_methodList['<%'] = IsCommand_1;
CheckTag_methodList['<%-'] = IsCommand_1;
CheckTag_methodList['<%='] = IsCommand_1;
//==============================================================================
...
CheckTag_methodList['(%'] = IsCommand_2;
CheckTag_methodList['(%-'] = IsCommand_2;
CheckTag_methodList['(%='] = IsCommand_2;
